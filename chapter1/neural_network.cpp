...
void neural_network::update_mini_batch(data_set::const_iterator it0,
  ...
  for (auto i = it0; i != it1; ++i) {
    auto delta = backprop(i->first, i->second);
    for (auto j = 0; j < nabla_b.size(); ++j) {
      nabla_b[j] += delta.first[j];
    }
    for (auto j = 0; j < nabla_w.size(); ++j) {
      nabla_w[j] += delta.second[j];
    }
  }
  for (auto i = 0; i < biases_.size(); ++i) {
    biases_[i] -= eta / (it1 - it0) * nabla_b[i];
  }
  for (auto i = 0; i < weights_.size(); ++i) {
    weights_[i] -= eta / (it1 - it0) * nabla_w[i];
  }
}

std::pair<std::vector<vector>, std::vector<matrix>>
neural_network::backprop(vector activation, const vector& label) {
